...
-- GUI Setup local gui = Instance.new("ScreenGui", LocalPlayer:WaitForChild("PlayerGui")) local frame = Instance.new("Frame", gui) frame.Position = UDim2.new(0, 10, 0, 50) frame.Size = UDim2.new(0, 200, 0, 200) frame.BackgroundColor3 = Color3.fromRGB(30, 30, 30) frame.BorderSizePixel = 0

local function addToggle(name, index, toggleKey) local btn = Instance.new("TextButton", frame) btn.Size = UDim2.new(1, -10, 0, 30) btn.Position = UDim2.new(0, 5, 0, 5 + index * 35) btn.BackgroundColor3 = Color3.fromRGB(50, 50, 50) btn.TextColor3 = Color3.new(1, 1, 1) btn.Text = name .. ": OFF" btn.MouseButton1Click:Connect(function() toggles[toggleKey] = not toggles[toggleKey] btn.Text = name .. (toggles[toggleKey] and ": ON" or ": OFF") end) end

local function addButton(name, index, callback) local btn = Instance.new("TextButton", frame) btn.Size = UDim2.new(1, -10, 0, 30) btn.Position = UDim2.new(0, 5, 0, 5 + index * 35) btn.BackgroundColor3 = Color3.fromRGB(70, 30, 30) btn.TextColor3 = Color3.new(1, 1, 1) btn.Text = name btn.MouseButton1Click:Connect(callback) end

addToggle("Aimlock", 0, "aimlock") addToggle("AutoBlock", 1, "autoblock") addToggle("AutoFly", 2, "autofly") addButton("Invincibility Reset", 3, function() local char = LocalPlayer.Character if char and char:FindFirstChild("HumanoidRootPart") then lastPosition = char.HumanoidRootPart.Position char:BreakJoints() task.wait(1) LocalPlayer:LoadCharacter() task.wait(1) local newChar = LocalPlayer.Character if newChar and newChar:FindFirstChild("HumanoidRootPart") then newChar:MoveTo(lastPosition) end end end)
